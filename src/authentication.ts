...
export default class AuthenticationHandler {
  ...
  constructor() {
    ...
    database.query({ 
      query: "CREATE TABLE IF NOT EXISTS logins (id INT(10) PRIMARY KEY AUTO_INCREMENT, username VARCHAR(255) NOT NULL, hash VARCHAR(255) NOT NULL, salt VARCHAR(255) NOT NULL)"
     })

    console.log(`Loaded JWT authentication handler with hashing secret ${Buffer.from(this.secret).toString("base64")}`)
  }
  ...
  createToken({ username, exp }: JwtClaim): string {
    ...
    // TODO: asymmetric RSA encryption
  ...
  async validateToken(token: string): Promise<JwtClaim | null> {
    ...
    if (signature == newSignature) {
      return JSON.parse(Buffer.from(claim, "base64").toString('utf-8')) as JwtClaim
    }
